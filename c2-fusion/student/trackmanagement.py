...
class Track:
    ...
    def __init__(self, meas: Measurement, id):
        ...
        T_sensor_to_veh = meas.sensor.sens_to_veh
        z_veh = T_sensor_to_veh * np.asmatrix(np.vstack((meas.z, [1])))
        self.x = np.zeros((6, 1))
        self.x[0:3] = z_veh[0:3]

        self.P = np.asmatrix(np.zeros((6, 6)))
        P_pos = M_rot * meas.R * M_rot.T
        P_vel = np.asmatrix(np.diag([params.sigma_p44**2, params.sigma_p55**2, params.sigma_p66**2]))
        self.P[0:3, 0:3] = P_pos
        self.P[3:6, 3:6] = P_vel
        self.state = params.state_init
...
class Trackmanagement:
    ...
    def manage_tracks(self, unassigned_tracks, unassigned_meas, meas_list):
        ...
        for track in self.track_list:
            if track.state == params.state_conf:
                threshold = params.delete_threshold
            else:
                threshold = params.tentative_threshold
